...
pub async fn start(ctx: Context, message: Message) -> CommandResult {
    ...
    if let Some(encoded_params) = split.next() {
        ...
        if registered {
            ...
            let welcome_text = escape_markdown(
                &"Welcome to REF Bot, your gateway to a Premier Telegram Referral Bot! \
                Start referring friends and join contests with /help.\n\n\
                Join our Telegram Group: https://t.me/refereeP to stay updated for our upcoming presale and token launch!", None
            );
            let mut welcome_message = SendMessage::new(sender_id, &welcome_text);
            welcome_message.set_parse_mode(&ParseMode::MarkdownV2);
            ctx.api.send_message(welcome_message).await?;
        }
    }
...
// leaderboard implementation not fully functional yet, Work IP

// #[command(description = "Show the top 3 users with the most invites")]
// pub async fn leaderboard(ctx: Context, message: Message) -> CommandResult {
//     let chatid = message.chat.id();
//     let leaderboard_data = {
//         let guard = ctx.data.read().await;
//         let map = guard.get::<DBKey>().expect("Expected DBKey in data map");
//         let conn = map.get().unwrap();

//         let mut stmt = conn.prepare("
//             SELECT source, COUNT(DISTINCT contest) AS participation_count
//             FROM invitations
//             GROUP BY source
...
//             Ok((row.get::<, i64>(0)?, row.get::<_, i64>(1)?))
//         }).unwrap();

//         leaderboard_iter.map(|result| result.unwrap()).collect::<Vec<(i64, i64)>>()
//     };

//     let text = if leaderboard_data.is_empty() {
//         "Currently, there are no rankings to display.".to_string()
//     } else {
//         let mut msg = "Top 3 Leaders:\n\n".to_string();
//         for (index, (user_id, participation_count)) in leaderboard_data.iter().enumerate() {
//             // Ideally, fetch user names or usernames here, but for simplicity, using user IDs
//             msg += &format!("{}. User ID: {}, Participations: {}\n", index + 1, user_id, participation_count);
//         }
//         msg
//     };

//     let mut reply = SendMessage::new(chat_id, &escape_markdown(&text, None));
//     reply.set_parse_mode(&ParseMode::MarkdownV2);
//     let res = ctx.api.send_message(reply).await;
//     if res.is_err() {
//         let err = res.err().unwrap();
